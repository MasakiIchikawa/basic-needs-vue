const contents = {
	messages: {
		...
		sentence: {
			...
			start_description:'図のように、それぞれの質問について\n「よく当てはまる」から「全く当てはまらない」\nの５段階の回答が表示されますので、\n一番当てはまるものを選択してください。',
			start_description2:'選択すると自動的に次の質問が表示されます。\n質問は51問(回答時間：3分程度)です。',
			privacy_comment:'初めて利用される方は最後に誕生年・職業などの簡単なアンケートがあります。',
			email_note:'必ずメール受信できるアドレスを登録してください。登録したメールアドレスに結果ページへのURLをお送りします。メール受信できない場合、うまくテスト結果を提供できない可能性があります。',
			...
			ct_content:'基本的欲求の概念は、アメリカの精神科医であったウィリアム・グラッサーの提唱する選択理論心理学の一部です。基本的欲求の概念について詳しくお知りになりたい方はぜひワンデーセミナーにご参加ください。',
			...
			result_alert:'結果数値は大きければいいわけではないのでご注意ください。\n欲求が強いと、その欲求がなかなか満たされないこともあり、\n逆に欲求が弱いとその欲求は満たしやすいと判断できます。',
			loading:'しばらくお待ちください'
		},
		validate:{
